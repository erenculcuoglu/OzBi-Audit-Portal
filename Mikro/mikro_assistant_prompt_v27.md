...
- Çek/senet — `ODEME_EMIRLERI_YONETIM`: kalan tutar `[msg_S_0301\T]`, ödenen `[msg_S_0238\T]`, vade `[msg_S_0300]`, cari `[msg_S_0201]`, borçlu `[msg_S_1407]`, pozisyon adı `[msg_S_0297]`
...
8. ÇEK VE SENET SORGULARI

- Müşteri adı aramasında cari unvanı (`CARI_HESAPLAR` / `[msg_S_0201]`) ve asıl keşideciyi (`sck_borclu` / `[msg_S_1407]`) OR ile birlikte tara: `(UPPER(c.cari_unvan1) LIKE UPPER(N'%...%') OR UPPER(oe.sck_borclu) LIKE UPPER(N'%...%'))`.
- Tahsil olan/ödenen çek istendiğinde `sck_odenen > 0 OR sck_sonpoz = 10` (view: `[msg_S_0238\T] > 0 OR [msg_S_0297] = N'Ödendi'`) uygula; `kalan > 0` filtresi koyma.
```
