# Mikro ERP Ek Talimatı — v27.4 (Yalın & Pozitif Mimari)

Şema otoriterdir. Metadata'daki hesaplama, filtre ve ilişkileri doğrudan uygula. Bu yönerge, Mikro ERP v27.4 iş mantığı ve finansal düşünme standartlarını tanımlar.
...
10. T-SQL HESAPLAMA VE GRUPLAMA STANDARTLARI
- Çok Kademeli ve Süre Hesaplamaları: Evrak/satır bazlı gün ve süre hesaplamalarını (fatura bazında vade farkı, DSO vb.) `WITH ... AS` (CTE) içinde satır bazında hesapla; ana sorguda bu değerler üzerinden `AVG()`, `SUM()`, `MIN()` veya `MAX()` aggregate fonksiyonlarını uygula.
- Güvenli Tarih Dönüşümü: `cha_vade` ve `cha_tarihi` gibi tamsayı tarih alanlarını `TRY_CONVERT(date, CONVERT(char(8), cha_vade), 112)` ile dönüştür.
- Sıfıra Bölme Koruması: Oran, ortalama ve birim fiyat hesaplamalarında paydayı `NULLIF(kolon, 0)` ile koru.
- Gruplama Bütünlüğü: `SELECT` listesindeki tüm tekil ve boyutsal kolonları `GROUP BY` ifadesine eksiksiz dahil et.
```
...
- Süre ve ortalama vade analizlerinde CTE bazlı katmanlı hesaplama sonucunu net göster.
```
