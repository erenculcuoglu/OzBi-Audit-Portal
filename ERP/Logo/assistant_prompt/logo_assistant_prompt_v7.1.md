# OzBI Logo ERP Ek Talimatı — v7.1
...
- Banka ve Kasa Yön Standartları (`BNFLINE`, `KSLINES`):
  - Banka/Kasaya Para Girişi: `SIGN = 0` (Borç). Ör: Gelen Havale/EFT (`TRCODE = 3`), Nakit Tahsilat (`TRCODE = 11`), Verilen Hizmet (`TRCODE = 17`).
  - Banka/Kasadan Para Çıkışı: `SIGN = 1` (Alacak). Ör: Gönderilen Havale/EFT (`TRCODE = 4`), Nakit Ödeme (`TRCODE = 12`), Çek/Senet Ödemesi (`TRCODE IN (18, 19)`).
  - Gelen havale/EFT sorgularında doğrudan `TRCODE = 3` filtresini kullan; `BNFLINE` tablosunda gelen havaleler daima `SIGN = 0` (Giriş) olarak temsil edilir.
- Fatura Tutar Semantiği (`INVOICE`):
  - `GROSSTOTAL`: KDV hariç indirimsiz mal/hizmet brüt tutarı (`[Brüt Tutar]` veya `[KDV Hariç Tutar]`).
  - `TOTALDISCOUNTED`: KDV hariç indirimli net matrah (`[Net Matrah]` veya `[İndirimli Tutar]`).
  - `TOTALVAT`: Toplam KDV tutarı (`[KDV Tutarı]`).
  - `NETTOTAL`: KDV dahil ödenecek nihai genel toplam (`[Genel Toplam]` veya `[KDV Dahil Tutar]`).
